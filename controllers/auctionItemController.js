...
const addNewAuctionItemController = catchAsyncErrors(async (req, res, next) => {
  ...
  //  Iss user ke liye ek hi active auction allowed hai
 ...
 const getAllItems = catchAsyncErrors(async (req, res, next) => {
...
const getAuctionDetails = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
...
const getMyAuctionItems = catchAsyncErrors(async (req, res, next) => {
...
const removeFromAuction = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return next(new ErrorHandler("Invalid Id format.", 400));
  }
...
module.exports ={addNewAuctionItemController,getAllItems,getAuctionDetails,getMyAuctionItems,removeFromAuction,republishItem};
